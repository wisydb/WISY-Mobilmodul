...
function updateMainnav() {
	...
	// Gek�rzte Texte erst zu- dann bei Klick aufklappen
	...
	// Sonderfall Kontakt / Anbieter vcard: Alle folgenden Elemente im �bergeordneten Container zu- / aufklappen
...
function prepareSearch() {
	...
	$('#wisy_umkreis').on('change', function() {
		// vorherigen umkreis:xxx/yyy und km:zzz l�schen
		...
		if(km > 0) {
			...
			// TODO: Feld und Buttons sperren f�r x Sekunden und Spinner zeigen mit Text "Ortung" oder so
		} else {
...
function locationError(error) {
	alert('Es konnte keine Ortung durchgef�hrt werden. Fehlercode: ' + error.code);
}
...
function prepareSort() {
	...
	$('.wisy_select_order input').on('click', function() {
		...
		if(name == 'order_dn') {
			// Selectfeldwert auslesen und Name löschen, damit es nicht submitted wird
...
function swipe(direction) {
	...
	if($body.hasClass('wisyp_homepage')) {
		...
		// Suchergebnisliste -> Paging vor zur�ck oder back wenn right auf Seite 1
		if(direction == 'right') {
...
function describeFeedback()
{
	...
	else
	{
		...
		ajaxFeedback(0, descr); // Kommentar zur Bewertung hinzuf�gen; die Bewertung selbst (erster Parameter) wird an dieser Stelle ignoriert!
	}
}
