...
class MOBIL_SEARCH_RENDERER_CLASS extends WISY_SEARCH_RENDERER_CLASS
{
    ...
    function renderKursliste(&$searcher, $queryString, $offset)
    {
        ...
        if( $info['changed_query'] || sizeof((array) $info['suggestions']) )
        {
            ...
            {
                // <a href="'.$this->framework->getUrl('search', array('q'=>$info['changed_query'])).'"></a>
                echo '<b>Hinweis:</b> Der Suchauftrag wurde abge&auml;ndert in <i>'.isohtmlspecialchars($info['changed_query']).'</i>';
            ...
            if( sizeof((array) $info['suggestions']) )
            {
                echo '<h3>Leider nichts gefunden.</h3>';
                echo '<ul>';
